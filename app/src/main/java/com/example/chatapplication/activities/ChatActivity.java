...
public class ChatActivity extends BaseActivity {
       // private ActivityChatApplicationBinding binding;
    ...
    private void sendMessage()
    {
        ...
        if(!isReceiverAvailable)
        {
            try{
                ...
                body.put(Constants.REMOTE_MCG_DATE,data);
                body.put(Constants.REMOTE_MCG_REGISTRATION_IDS,tokens);
        ...
        database.collection(Constants.KEY_COLLECTION_USERS).document(receiverUser.id)
                .addSnapshotListener(ChatActivity.this,(value , error) ->
                {
                    ...
                    if(value != null)
                    {
                        ...
                        {
                            int availability= Objects.requireNonNull(value.getLong(Constants.KEY_AVAILABLITY)).intValue();
            ...
            public void onResponse(@NonNull Call<String> call, @NonNull Response<String> response) {
                if(response.isSuccessful())
                {
                    try{
                        ...
                        {
                            JSONObject responceJson = new JSONObject(response.body());
                            JSONArray reesults = responceJson.getJSONArray("results");
                            if(responceJson.getInt("failure") == 1)
                            {
                                JSONObject error = (JSONObject) reesults.get(0);
